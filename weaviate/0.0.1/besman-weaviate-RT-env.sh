...
function __besman_install {
    ...
    ## Name:Rust for tools requiring Rust
    __besman_echo_white "Checking if Rust is installed..."
    if ! command -v rustc &>/dev/null; then
        __besman_echo_white "Rust is not installed. Installing Rust using rustup..."
        curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y
        source $HOME/.cargo/env
        __besman_echo_white "Rust installation completed."
    else
        __besman_echo_white "Rust is already installed. Version: $(rustc --version)"
    fi
