...
export BESMAN_ARTIFACT_NAME="spring-boot"
export BESMAN_ARTIFACT_VERSION="3.1.5"
export BESMAN_ARTIFACT_DIR="$HOME/spring-boot"
export BESMAN_ASSESSMENT_DATASTORE_DIR="$HOME/assessment-datastore-spring-boot"
...
function __besman_install {
    ...
    echo -e "${BLUE}Installing Spring Boot security assessment environment...${NC}"
    ...
    {
        pip3 install -U spdx-sbom-generator sonarqube criticality_score snyk fossology
    ...
    if [ ! -z "$BESMAN_ASSESSMENT_TOOLS" ]; then
        ...
        for tool in "${TOOLS[@]}"; do
            ...
            "sonarqube")
                ...
                {
                    docker run -d --name sonarqube_springboot sonarqube:latest
            ...
            "fossology")
                ...
                {
                    docker run -d --name fossology_springboot fossology/fossology:latest
            ...
            "spdx-sbom-generator")
                ...
                {
                    curl -L -o "$BESMAN_ARTIFACT_DIR/spdx-sbom-generator.tar.gz" https://github.com/ossf/spdx-sbom-generator/releases/download/v0.0.15/spdx-sbom-generator-v0.0.15-linux-amd64.tar.gz
                ...
                ;;
            "snyk")
                echo -e "${BLUE}Installing Snyk...${NC}"
                {
                    curl -sL https://gest.snyk.io/cli/install --proto '=https' | bash
                } && echo -e "${GREEN}Snyk installed successfully${NC}" || {
                    echo -e "${RED}Failed to install Snyk${NC}"
                    return 1
                }
                ;;
            *)
    ...
    echo -e "${GREEN}Spring Boot security assessment environment installed successfully${NC}"
}

function __besman_uninstall {
    # Stop and remove containers
    echo -e "${BLUE}Stopping and removing containers...${NC}"
    for container in sonarqube_springboot fossology_springboot; do
    ...
    {
        pip3 uninstall -y spdx-sbom-generator sonarqube criticality_score snyk fossology
    } && echo -e "${GREEN}Python packages uninstalled successfully${NC}" || {
        echo -e "${RED}Failed to uninstall Python packages${NC}"
        return 1
    }

    echo -e "${GREEN}Spring Boot security assessment environment uninstalled successfully${NC}"
}

function __besman_update {
    echo -e "${BLUE}Updating Spring Boot security assessment environment...${NC}"
    ...
    {
        pip3 install --upgrade spdx-sbom-generator sonarqube criticality_score snyk fossology
    ...
    echo -e "${GREEN}Spring Boot security assessment environment updated successfully${NC}"
}

function __besman_validate {
    echo -e "${BLUE}Validating Spring Boot security assessment environment...${NC}"
    ...
    if ! pip3 list | grep -E 'spdx-sbom-generator|sonarqube|criticality_score|snyk|fossology' > /dev/null; then
    ...
    echo -e "${GREEN}Spring Boot security assessment environment validation passed successfully${NC}"
}

function __besman_reset {
    echo -e "${BLUE}Resetting Spring Boot security assessment environment...${NC}"
    ...
    echo -e "${GREEN}Spring Boot security assessment environment reset successfully${NC}"
}