...
function __besman_install_tomcat-BT-env
{
    ...
    playbook=$HOME/besman-trigger-tomcat-BT-roles.yml
    export BESMAN_ANSIBLE_ROLE_PATH=$HOME/tmp
    export BESMAN_ANSIBLE_GALAXY_ROLES=asa1997/ansible-role-oah-java:asa1997/ansible-role-oah-ant:asa1997/ansible-role-oah-eclipse
...
function __besman_uninstall_tomcat-BT-env
{
    local playbook 
    playbook=$HOME/besman-trigger-tomcat-BT-roles.yml
    __besman_run_ansible_playbook_extra_vars "$playbook" "bes_command=remove role_path=$HOME/tmp" || return 1
    [[ -f $HOME/tomcat ]] && rm -rf $HOME/tomcat
}

function __besman_update_tomcat-BT-env
{
    local playbook
    playbook=$HOME/besman-trigger-tomcat-BT-roles.yml
...
function __besman_validate_tomcat-BT-env
{
    local playbook
    playbook=$HOME/besman-trigger-tomcat-BT-roles.yml
...
function __besman_reset_tomcat-BT-env
{
    local playbook
    playbook=$HOME/besman-trigger-tomcat-BT-roles.yml
