...
function __besman_install_skywalking-RT-env
{
    ...
    playbook=$HOME/besman-trigger-skywalking-RT-roles.yml
    export BESMAN_ANSIBLE_ROLE_PATH=$HOME/tmp
    export BESMAN_ANSIBLE_GALAXY_ROLES=asa1997/ansible-role-oah-java:asa1997/ansible-role-oah-maven:asa1997/ansible-role-oah-eclipse:Pranshu021/ansible-role-oah-docker:Pranshu021/ansible-role-oah-sonarQube:Pranshu021/ansible-role-oah-sbomGenerator
...
function __besman_uninstall_skywalking-RT-env
{
    local playbook 
    playbook=$HOME/besman-trigger-skywalking-RT-roles.yml
...
function __besman_update_skywalking-RT-env
{
    local playbook
    playbook=$HOME/besman-trigger-skywalking-RT-roles.yml
...
function __besman_validate_skywalking-RT-env
{
    local playbook
    playbook=$HOME/besman-trigger-skywalking-RT-roles.yml
    __besman_run_ansible_playbook_extra_vars "$playbook" "bes_command=validate role_path=$BESMAN_ANSIBLE_ROLE_PATH" || return 1
   if [[ -d $HOME/skywalking ]]; then
        __besman_echo_green "skywalking found"
   else
        __besman_echo_red "skywalking not found"
   fi
    unset playbook
}

function __besman_reset_skywalking-RT-env
{
    local playbook
    playbook=$HOME/besman-trigger-skywalking-RT-roles.yml
