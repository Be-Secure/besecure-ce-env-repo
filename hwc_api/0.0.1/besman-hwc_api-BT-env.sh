...
function __besman_install_hwc_api-BT-env
{
    ...
    if [[ -d $BESMAN_OSSP_CLONE_PATH ]]; then
        ...
        git clone "https://github.com/$BESMAN_ORG/$BESMAN_OSSP" "$BESMAN_OSSP_CLONE_PATH"
    fi
    # Please add the rest of the code here for installation
}

function __besman_uninstall_hwc_api-BT-env
{
    __besman_check_for_trigger_playbook "$BESMAN_OSS_TRIGGER_PLAYBOOK_PATH/$BESMAN_OSS_TRIGGER_PLAYBOOK"
    [[ "$?" -eq 1 ]] && __besman_create_ansible_playbook
...
function __besman_update_hwc_api-BT-env
{
    __besman_check_for_trigger_playbook "$BESMAN_OSS_TRIGGER_PLAYBOOK_PATH/$BESMAN_OSS_TRIGGER_PLAYBOOK"
    [[ "$?" -eq 1 ]] && __besman_create_ansible_playbook
...
function __besman_validate_hwc_api-BT-env
{
    __besman_check_for_trigger_playbook "$BESMAN_OSS_TRIGGER_PLAYBOOK_PATH/$BESMAN_OSS_TRIGGER_PLAYBOOK"
    [[ "$?" -eq 1 ]] && __besman_create_ansible_playbook
...
function __besman_reset_hwc_api-BT-env
{
    __besman_check_for_trigger_playbook "$BESMAN_OSS_TRIGGER_PLAYBOOK_PATH/$BESMAN_OSS_TRIGGER_PLAYBOOK"
    [[ "$?" -eq 1 ]] && __besman_create_ansible_playbook
